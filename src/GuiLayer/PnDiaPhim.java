...
import DataAccessLayer.ModelTableDiaPhim;
 ...
 *         panel kho đĩa phim
 */
public class PnDiaPhim extends JPanel {
	// gui field
	private JTextField tfMaSP, tfTenSP, tfDienVien, tfDaoDien, tfTheLoai, tfGianHang, tfSoLuong, tfGiaBan, tfGiaMua, tfChietKhau, tfSearch;
	...
	// data field
	private DataDiaPhim data;
	private DiaPhim selectedProduct;
	private ModelTableDiaPhim modelTable;
	...
	public PnDiaPhim(NhanVien user, DataDiaPhim data) {
		// khởi tạo dữ liệu
		this.data = data;
		currentUserIsAdmin = user.isAdmin();
		modelTable = new ModelTableDiaPhim(data.getList());
	...
	//	setBorder(new EmptyBorder(1, 1, 1, 1));
		...
		// label kho dia phim
		JLabel lnKhoDiaPhim = new JLabel("KHO ĐĨA PHIM");
		lnKhoDiaPhim.setAlignmentX(Component.CENTER_ALIGNMENT);
		lnKhoDiaPhim.setFont(new Font("Tahoma", Font.PLAIN, 27));
		add(lnKhoDiaPhim);
		...
		// label ma dia phim
		JLabel lbMaSP = new JLabel("Mã đĩa phim");
		...
		// label ten dia phim
		JLabel lbTenSP = new JLabel("Tên đĩa phim");
		...
		pnDienVien.setBorder(new EmptyBorder(3, 10, 3, 20));
		pnLeft.add(pnDienVien);
		pnDienVien.setLayout(new BorderLayout(0, 0));
		// label tac gia
		JLabel lbDienVien = new JLabel("Diễn viên");
		lbDienVien.setPreferredSize(new Dimension(100, 16));
		lbDienVien.setFont(new Font("Tahoma", Font.PLAIN, 14));
		pnDienVien.add(lbDienVien, BorderLayout.LINE_START);
		// text field ten tac gia
		tfDienVien = new JTextField();
		pnDienVien.add(tfDienVien, BorderLayout.CENTER);
		//
		...
		pnRight.add(pnDaoDien);
		pnDaoDien.setBorder(new EmptyBorder(3, 10, 3, 10));
		pnDaoDien.setLayout(new BorderLayout(0, 0));
		// label nxb
		JLabel lbDaoDien = new JLabel("Đạo diễn");
		lbDaoDien.setPreferredSize(new Dimension(100, 16));
		lbDaoDien.setFont(new Font("Tahoma", Font.PLAIN, 14));
		pnDaoDien.add(lbDaoDien, BorderLayout.LINE_START);
		// text field nxb
		tfDaoDien = new JTextField();
		pnDaoDien.add(tfDaoDien, BorderLayout.CENTER);
		//
	...
	private void addListener() {
		...
		// check ID khi gõ trong tf mã đĩa phim
		tfMaSP.addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent e) {
				if (data.checkByID(tfMaSP.getText())) {
					lbThongBao.setText("Mã đĩa phim đã tồn tại!");
					lbThongBao.setForeground(Color.RED);
					tfMaSP.setForeground(Color.RED);
				} else {
					lbThongBao.setText("Mã đĩa phim có thể dùng");
		...
		btnMua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (buyListener != null && selectedProduct!=null) {
					buyListener.onBuyProduct(new ChiTietHoaDon(selectedProduct, "Đĩa phim", 1));
					JOptionPane.showMessageDialog(null, "đĩa phim bạn chọn đã được thêm vào giỏ");
				} else {
	...
	/**
	 * thêm 1 đĩa phim vào data
	 */
	private void themSanPham() {
		DiaPhim temp = data.them(tfMaSP.getText(), tfTenSP.getText(), tfTheLoai.getText() + "", tfGianHang.getText(), tfSoLuong.getText(), tfGiaMua.getText(), tfGiaBan.getText(), tfChietKhau.getText(),
				tfDaoDien.getText() + "", tfDienVien.getText());
		if (temp != null) {
	...
	private void suaSanPham() {
		DiaPhim temp = data.sua(selectedProduct, tfMaSP.getText(), tfTenSP.getText(), tfTheLoai.getText() + "", tfGianHang.getText(), tfSoLuong.getText(), tfGiaMua.getText(), tfGiaBan.getText(),
				tfChietKhau.getText(), tfDaoDien.getText() + "", tfDienVien.getText());
		if (temp != null) {
			modelTable.fireTableDataChanged();
			selectedProduct = temp;
			table.setRowSelectionInterval(table.convertRowIndexToView(selectedRow), table.convertRowIndexToView(selectedRow));
