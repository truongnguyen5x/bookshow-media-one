...
import DataAccessLayer.ModelTableSach;
 ...
 *         panel kho sách
 */
public class PnSach extends JPanel {
	// gui field
	private JTextField tfMaSP, tfTenSP, tfTacGia, tfNXB, tfTheLoai, tfGianHang, tfSoLuong, tfGiaBan, tfGiaMua, tfChietKhau, tfSearch;
	...
	// data field
	private DataSach data;
	private Sach selectedProduct;
	private ModelTableSach modelTable;
	...
	public PnSach(NhanVien user, DataSach data) {
		// khởi tạo dữ liệu
		this.data = data;
		currentUserIsAdmin = user.isAdmin();
		modelTable = new ModelTableSach(data.getList());
		...
		// label kho sach
		JLabel lbKhoSach = new JLabel("KHO SÁCH");
		...
		// label ma sach
		JLabel lbMaSP = new JLabel("Mã sách");
		...
		// label ten sach
		JLabel lbTenSP = new JLabel("Tên sách");
		...
		pnTacGia.setBorder(new EmptyBorder(3, 10, 3, 20));
		pnLeft.add(pnTacGia);
		pnTacGia.setLayout(new BorderLayout(0, 0));
		// label tac gia
		JLabel lbTacGia = new JLabel("Tác giả");
		lbTacGia.setPreferredSize(new Dimension(100, 16));
		lbTacGia.setFont(new Font("Tahoma", Font.PLAIN, 14));
		pnTacGia.add(lbTacGia, BorderLayout.LINE_START);
		// text field ten tac gia
		tfTacGia = new JTextField();
		pnTacGia.add(tfTacGia, BorderLayout.CENTER);
		//
		...
		pnRight.add(pnNXB);
		pnNXB.setBorder(new EmptyBorder(3, 10, 3, 10));
		pnNXB.setLayout(new BorderLayout(0, 0));
		// label nxb
		JLabel lbNXB = new JLabel("Nhà xuất bản");
		lbNXB.setPreferredSize(new Dimension(100, 16));
		lbNXB.setFont(new Font("Tahoma", Font.PLAIN, 14));
		pnNXB.add(lbNXB, BorderLayout.LINE_START);
		// text field nxb
		tfNXB = new JTextField();
		pnNXB.add(tfNXB, BorderLayout.CENTER);
		//
	...
	private void addListener() {
		...
		// check ID khi gõ trong tf mã sách
		tfMaSP.addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent e) {
				if (data.checkByID(tfMaSP.getText())) {
					lbThongBao.setText("Mã sách đã tồn tại!");
					lbThongBao.setForeground(Color.RED);
					tfMaSP.setForeground(Color.RED);
				} else {
					lbThongBao.setText("Mã sách có thể dùng");
		...
		btnMua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (buyListener != null && selectedProduct!=null) {
					buyListener.onBuyProduct(new ChiTietHoaDon(selectedProduct, "Sách", 1));
					JOptionPane.showMessageDialog(null, "sách bạn chọn đã được thêm vào giỏ");
				} else {
	...
	/**
	 * thêm 1 sách vào data
	 */
	private void themSanPham() {
		Sach temp = data.them(tfMaSP.getText(), tfTenSP.getText(), tfTheLoai.getText() + "", tfGianHang.getText(), tfSoLuong.getText(), tfGiaMua.getText(), tfGiaBan.getText(), tfChietKhau.getText(),
				tfNXB.getText() + "", tfTacGia.getText());
		if (temp != null) {
	...
	private void suaSanPham() {
		Sach temp = data.sua(selectedProduct, tfMaSP.getText(), tfTenSP.getText(), tfTheLoai.getText() + "", tfGianHang.getText(), tfSoLuong.getText(), tfGiaMua.getText(), tfGiaBan.getText(),
				tfChietKhau.getText(), tfNXB.getText() + "", tfTacGia.getText());
		if (temp != null) {
			modelTable.fireTableDataChanged();
			selectedProduct = temp;
			table.setRowSelectionInterval(table.convertRowIndexToView(selectedRow), table.convertRowIndexToView(selectedRow));
